...
| **every** entity fails with "Could not load metadata: 500 Internal Server Error" | **hit testing locally, not a backend bug** — the `$metadata` response body was `Error: getaddrinfo ENOTFOUND replace-with-your-s4-host`: `ui5.yaml`'s `fiori-tools-proxy` placeholder host was never filled in before running `npm start`, so the local proxy couldn't resolve *any* host and every OData call failed identically. Fix: fill in the real host/port/client in `ui5.yaml` (see "Run locally first" below), or skip `npm start` entirely and open the actually-deployed BSP app's URL instead. |
| a request to `$batch` gets `403` with `"CSRF token is invalid"` | **not a bug** — this is the OData V4 model's normal first-request handshake (the response header `x-csrf-token: Required` tells it to fetch a token and retry). The model retries automatically; look at the *next* request in the trace, not this one, to see whether data actually came back. |
| console shows `TypeError: Cannot read properties of undefined (reading '_cardIndex')` at `Dashboard.controller.js` `_renderCards` | **hit on first real data test, fixed** — the `aCards.forEach(...)` callback in `_renderCards` referenced `this._cardIndex` without `.bind(this)`; in strict mode an unbound callback's `this` is `undefined`. Fixed by adding `.bind(this)`. If you see this again anywhere else, it's the same class of mistake — grep the file for a `function (` callback that uses `this.` but has no matching `.bind(this)`. |
