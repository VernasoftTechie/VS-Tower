...
| **3/4** *(refined, 2026-09-05)* | 🔄 **Pending activation** | **Manager-facing increment**: `Owner` (`SDLUNAME`) added to `BackgroundJobHealth`; new `ZC_TWR_TRANSPORT_BY_OWNER` (`TransportByOwner`) — open vs. released TR count per developer/consultant ID. Zero new custom DDIC — reuses `ZI_TWR_TRANSPORT`'s already-confirmed fields and the `WorkItemSummary` 2-dim `GROUP BY` shape. | §14 (K), §20 (Q) | None |
...
| — | 🔄 **In progress** — first deploy attempted 2026-09-05 (2 tooling issues found/fixed), then rebuilt to match a client-approved mockup, not yet re-deployed/verified visually since | Freestyle dashboard shell (`ui/controltower`), rebuilt 2026-09-05 around a technical-manager use case per an approved click-through mockup: 11 uniform, chart-only cards (no dashboard tables), each clickable into a `sap.m.Dialog` detail list; `sap.viz.VizFrame` replaced with hand-rolled SVG/CSS charts; "by owner" bar charts on both Transport and Background Jobs. See `docs/04_fiori_ui_design.md` §2/§5 and `ui/README.md` for the current structure | §21 (R) | — |
