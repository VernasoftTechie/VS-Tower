...
| `USR02` — `TRDAT` (2026-09-09, `ZI_TWR_SEC_USER` extension) | `TRDAT` was already read (mislabelled `LastPasswordChangeDate` — it is actually **last-logon date**). Added `LastLogonDate` + a `LogonRecency` CASE bucket (0-30 / 31-90 / 90+ days / Never) for the "Active IDs" card. `dats_days_between` in a SELECT-list CASE (not WHERE — rule #27 OK); `trdat is initial` guards the blank-date case; each CASE branch cast to `abap.char(10)` for one result type. `ZC_TWR_SEC_ACTIVE` groups by `UserType` × `LogonRecency` where `IsLocked <> 'X'`. **Pending first activation** — additive change to a live view, low risk. |
| `E070` / `E071` / `TADIR` (2026-09-09, `ZC_TWR_LOCKED_OBJ*`) | No new tables — `ZC_TWR_LOCKED_OBJ` / `ZC_TWR_LOCKED_OBJ_BY_OWNER` are plain projections / GROUP-BY over the **existing** `ZI_TWR_STALE_OBJ` join, just without the `AgeInDays >= 180` filter (every open lock, not only the stale ones). Zero new field-name risk. |
| `RSAU_*` / Security Audit Log (2026-09-09, `ZCL_TWR_AUTHFAIL_QRY`) | The **second** ABAP class (D1's zero-class rule now used its two exceptions — no more). SAL is not CDS-readable, so a custom entity `ZC_TWR_AUTH_FAIL` + query class does a **dynamic** `CALL FUNCTION 'RSAU_READ_LOG'` (variable FM name, `PARAMETER-TABLE`, generic result table via `CREATE DATA ... TYPE STANDARD TABLE OF ('RSAUENTR2')`, `pick_component` for the fields) — **every identifier is a guess** (`RSAU_READ_LOG`, `IF_DATE_FROM/TO`, `ET_LOG`, `RSAUENTR2`, `SLG*` field names). All wrapped in `TRY … CATCH cx_root` so it can only ever yield an **empty card**, never an activation failure or a 500. If the card stays empty: SAL is off (SM19), unreadable by the dashboard user, or the FM signature differs — send the SE37 Import/Tables tabs of `RSAU_READ_LOG` (and confirm SAL is DB-logged) and `build_list( )` maps straight onto it. |
| `SNAP` (2026-09-08, `ZCL_TWR_SHORTDUMP_QRY`) | Read in ABAP, not CDS. **Confirmed selectable** on this S/4 release — the class activated and returned ~300 dump rows. Fields used: `DATUM UZEIT UNAME AHOST XHOLD` (all fine). The readable bits (runtime-error name / short text / program) are **not** SNAP columns — the class enriches them best-effort via a **dynamic** `CALL FUNCTION 'RS_ST22_GET_DUMPS'` (variable FM name + `PARAMETER-TABLE`, so a missing FM / wrong interface is a *caught runtime error*, never an activation failure; params `P_DAYFR`/`P_DAYTO`/`P_LIST` and the returned structure are all guesses). If those columns render as "–" after activation, get the SE37 signature of `RS_ST22_GET_DUMPS` (Import + Tables tabs) and wire `enrich_from_st22( )` exactly. |
