...
| `SWWWIHEAD` | `WI_ID WI_TYPE WI_STAT` — **not yet pulled/activated.** Client confirmed the table is in force (Workflow is actively used) but did not verify individual field names via SE11. Moderate confidence, not `TBTCO`/`E070`-level certain — see `00_context_and_decisions.md` §8. `SWWUSERWI` (agent assignment) deliberately not touched yet; dates (`WI_CD`/`WI_CT`, unverified names) deliberately not included in this first cut either. |
